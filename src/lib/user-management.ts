...
export class UserManagementService {
  ...
  async suspendUser(
    ...
    try {
      ...
      const suspender = await prisma.users.findFirst({
        where: { 
          id: suspendedBy,
          tenantId 
        }
  ...
  async reactivateUser(
    ...
    try {
      ...
      const reactivator = await prisma.users.findFirst({
        where: { 
          id: reactivatedBy,
          tenantId 
        }
  ...
  async removeUser(
    ...
    try {
      ...
      const remover = await prisma.users.findFirst({
        where: { 
          id: removedBy,
          tenantId 
        }
  ...
  async canAddUser(tenantId: string): Promise<{ allowed: boolean; reason?: string }> {
    try {
      const currentUserCount = await prisma.users.count({
        where: { 
          tenantId,
          status: { in: ['ACTIVE'] }
        }
